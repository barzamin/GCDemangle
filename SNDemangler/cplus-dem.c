...
/* Modified by Martin Day, SN Systems Ltd. to make it build without ansidecl and to use our own interface */

/* Modified by Martin Day, SN Systems Ltd. to cope with variable and function names in anonymous name spaces */
...
#define DEMANGLE_STYLE_MASK  (DEMANGLE_OPT_AUTO + DEMANGLE_OPT_GNU + DEMANGLE_OPT_LUCID + DEMANGLE_OPT_ARM + DEMANGLE_OPT_HP + DEMANGLE_OPT_EDG)
...
enum demangling_styles current_demangling_style = DEMANGLE_OPT_GNU;
...
#define PRINT_ANSI_QUALIFIERS (work -> options & DEMANGLE_OPT_ANSI)
#define PRINT_ARG_TYPES       (work -> options & DEMANGLE_OPT_PARAMS)
...
#define SCOPE_STRING(work) ((work->options & DEMANGLE_OPT_JAVA) ? "." : "::")
...
/* Takes operator name as e.g. "++" and returns mangled
   ...
   If OPTIONS & DEMANGLE_OPT_ANSI == 1, return the ANSI name;
   if OPTIONS & DEMANGLE_OPT_ANSI == 0, return the old GNU name.  */
  ...
  for (i = 0; i < sizeof (optable) / sizeof (optable[0]); i++)
    {
	  ...
	  && (options & DEMANGLE_OPT_ANSI) == (optable[i].flags & DEMANGLE_OPT_ANSI)
...
/* char *cplus_demangle (const char *mangled, int options)
   ...
   The OPTIONS arg may contain one or more of the following bits:
   	...
   	DEMANGLE_OPT_ANSI	ANSI qualifiers such as `const' and `void' are
			included.
	DEMANGLE_OPT_PARAMS	Function parameters are included.

   For example,

   cplus_demangle ("foo__1Ai", DEMANGLE_OPT_PARAMS)		=> "A::foo(int)"
   cplus_demangle ("foo__1Ai", DEMANGLE_OPT_PARAMS | DEMANGLE_OPT_ANSI)	=> "A::foo(int)"
   cplus_demangle ("foo__1Ai", 0)			=> "A::foo"

   cplus_demangle ("foo__1Afe", DEMANGLE_OPT_PARAMS)		=> "A::foo(float,...)"
   cplus_demangle ("foo__1Afe", DEMANGLE_OPT_PARAMS | DEMANGLE_OPT_ANSI)=> "A::foo(float,...)"
...
cplus_demangle (const char *mangled, int options)
{
  ...
  memset ((char *) work, 0, sizeof (work));
  work -> options = options;
  if ((work -> options & DEMANGLE_STYLE_MASK) == 0)
    work -> options |= (int) current_demangling_style & DEMANGLE_STYLE_MASK;
  ...
  if ((mangled != NULL) && (*mangled != '\0'))
    {
	 ...
	 example.  */
      ...
      if (((current_demangling_style & DEMANGLE_OPT_AUTO) || (current_demangling_style & DEMANGLE_OPT_GNU)))
	{
  ...
  if (work->typevec != NULL)
    {
	  ...
	  //
	  // RGO WE HAVE JUST FREED THE STORAGE SO WE SHOULD
	  // ZERO THE SIZE COUNTER TOO - LN13281
	  work->typevec_size = 0;
    }
  if (work->tmpl_argvec)
    {
...
demangle_signature (struct work_stuff *work, const char **mangled, string *declp)
{
  ...
  while (success && (**mangled != '\0'))
	...
	case 'L':
	  ...
	  if ((current_demangling_style & DEMANGLE_OPT_HP))
	    {
	...
	case '5': case '6': case '7': case '8': case '9':
	  ...
	  if ((current_demangling_style & DEMANGLE_OPT_AUTO) || (current_demangling_style & DEMANGLE_OPT_GNU) || (current_demangling_style & DEMANGLE_OPT_EDG))
	    {
	...
	case 'F':
	  ...
	  if ((current_demangling_style & DEMANGLE_OPT_LUCID) || (current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP) || (current_demangling_style & DEMANGLE_OPT_EDG))
	    {
	      forget_types (work);
	    }
	  ...
	  if (success && ((current_demangling_style & DEMANGLE_OPT_AUTO) || (current_demangling_style & DEMANGLE_OPT_EDG)) && **mangled == '_')
	    {
	      ++(*mangled);
	...
	case '_':
	  if ((current_demangling_style & DEMANGLE_OPT_GNU) && expect_return_type)
	    {
	...
	case 'H':
	  if ((current_demangling_style & DEMANGLE_OPT_GNU))
	    {
	...
	*/
	...
	if (success && expect_func)
	  {
	    func_done = 1;
              if ((current_demangling_style & DEMANGLE_OPT_LUCID) || (current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_EDG))
                {
                  forget_types (work);
                }
	    success = demangle_args (work, mangled, declp);
...
demangle_template (struct work_stuff *work, const char **mangled, string *tname, string *trawname, int is_type, int remember)
{
  ...
  if (is_type)
      ...
      else
	{
	  ...
	  is_java_array = (work -> options & DEMANGLE_OPT_JAVA)
...
arm_pt (struct work_stuff *work, const char *mangled, int n, const char **anchor, const char **args)
{
  ...
  if (((current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP)) && (*anchor = mystrstr (mangled, "__pt__")) != 0)
    {
      int len;
      *args = *anchor + 6;
  ...
  if ((current_demangling_style & DEMANGLE_OPT_AUTO) || (current_demangling_style & DEMANGLE_OPT_EDG))
    {
      if ((*anchor = mystrstr (mangled, "__tm__")) != 0
          || (*anchor = mystrstr (mangled, "__ps__")) != 0
          || (*anchor = mystrstr (mangled, "__pt__")) != 0)
        {
          int len;
          *args = *anchor + 6;
          len = consume_count (args);
      ...
      else if ((*anchor = mystrstr (mangled, "__S")) != 0)
        {
 	  int len;
 	  *args = *anchor + 3;
 	  len = consume_count (args);
...
demangle_arm_hp_template (struct work_stuff *work, const char **mangled, int n, string *declp)
{
  ...
  /* Check for HP aCC template spec: classXt1t2 where t1, t2 are
     template args */
  if ((current_demangling_style & DEMANGLE_OPT_HP) && ((*mangled)[n] == 'X'))
    {
...
demangle_prefix (struct work_stuff *work, const char **mangled, string *declp)
{
  ...
  else if (((current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP) || (current_demangling_style & DEMANGLE_OPT_EDG)) && strncmp(*mangled, "__std__", 7) == 0)
    {
      /* it's a ARM global destructor to be executed at program exit */
      (*mangled) += 7;
      work->destructor = 2;
    }
  else if (((current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP) || (current_demangling_style & DEMANGLE_OPT_EDG)) && strncmp(*mangled, "__sti__", 7) == 0)
    {
  ...
  if (scan == NULL)
	       ...
	       || (scan[2] == 't') || (scan[2] == 'K') || (scan[2] == 'H')))
    {
	 ...
	 to them. As an extension to ARM demangling we handle this case.  */
      if (((current_demangling_style & DEMANGLE_OPT_LUCID) || (current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP))
  ...
  else if ((current_demangling_style & DEMANGLE_OPT_ARM) && scan[2] == 'p' && scan[3] == 't')
    {
  ...
  else if ((current_demangling_style & DEMANGLE_OPT_EDG) && ((scan[2] == 't' && scan[3] == 'm')
                              || (scan[2] == 'p' && scan[3] == 's')
                              || (scan[2] == 'p' && scan[3] == 't')))
    {
	 ...
	 */
      if (!((current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_LUCID) || (current_demangling_style & DEMANGLE_OPT_HP) || (current_demangling_style & DEMANGLE_OPT_EDG))
	  || (arm_special (mangled, declp) == 0))
	{
	  ...
	  if ((scan = mystrstr (scan, "__")) == NULL || (*(scan + 2) == '\0'))
	  ...
	  else
	    {
	      ...
	      if (!((current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_HP)))
		{
...
demangle_qualified (struct work_stuff *work, const char **mangled, string *result, int isfuncname, int append)
{
  ...
  while (qualifiers-- > 0)
    {
      int remember_K = 1;
	  //MAR - 06/07/01 - functions called below (eg demangle_template) reset the pointers to NULL
	  //				 so we need to free the memory here
	...
	{
	  if ((current_demangling_style & DEMANGLE_OPT_EDG))
            {
	      int namelength;
...
do_type (struct work_stuff *work, const char **mangled, string *result)
{
  ...
  while (success && !done)
	...
	case 'p':
	  (*mangled)++;
	  if (! (work -> options & DEMANGLE_OPT_JAVA))
	    string_prepend (&decl, "*");
	  if (tk == tk_none)
	    tk = tk_pointer;
	  break;
...
demangle_fund_type (struct work_stuff *work, const char **mangled, string *result)
{
  ...
  switch (**mangled)
      ...
      if (**mangled == '_')
	{
	  int i;
	  ++(*mangled);
          // SN-Phil: stop missing trailing '_' or big name causing scan past end of name
          i = 0 ;
          while ( ( i<9) && (**mangled != 0x00) && (**mangled != '_') )
          {
	    buf[i] = **mangled;
            ++(*mangled) ;
            i++ ;
    ...
    case 't':
      {
		...
		//MAR - 06/07/01 - The strings in btype were never being freed
        string_delete(&btype);
		//
        break;
      }
...
do_arg (struct work_stuff *work, const char **mangled, string *result)
{
  ...
  if (work->previous_argument)
  {
    ...
    //MAR - 06/07/01 - do_type will clear the pointers in previous_argument
	//					so free the memory for them (if any) here
    string_delete(work->previous_argument);
	//
  }
  else
      ...
      if ((**mangled == 'N') || (**mangled == 'T'))
	{
	  ...
	  if (temptype == 'N')
	  ...
	  else
	    {
          ...
          if (((current_demangling_style & DEMANGLE_OPT_HP) || (current_demangling_style & DEMANGLE_OPT_ARM) || (current_demangling_style & DEMANGLE_OPT_EDG)) && work -> ntypes >= 10)
            {
...
demangle_nested_args (struct work_stuff *work, const char **mangled, string *declp)
{
  ...
  if (work->previous_argument)
  {
	  ...
	  // MAR - 06/07/01 - We're about to overwrite previous_argument, so free the memory
      free ((char*) work->previous_argument);
	  //
  }
...
demangle_function_name (struct work_stuff *work, const char **mangled, string *declp, const char *scan)
{
  ...
  *(declp -> p) = '\0';
  ...
  if ((current_demangling_style & DEMANGLE_OPT_HP) && (**mangled == 'X'))
    {
...
static int flags = DEMANGLE_OPT_PARAMS | DEMANGLE_OPT_ANSI;
